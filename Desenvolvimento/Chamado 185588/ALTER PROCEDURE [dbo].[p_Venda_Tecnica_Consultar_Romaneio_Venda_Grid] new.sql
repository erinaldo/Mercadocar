USE [MCAR_NET]
GO
/****** Object:  StoredProcedure [dbo].[p_Venda_Tecnica_Consultar_Romaneio_Venda_Grid]    Script Date: 11/06/2019 13:53:50 ******/
SET ANSI_NULLS ON
GO
...
-- </history>
-------------------------------------------------------------------------------
ALTER PROCEDURE [dbo].[p_Venda_Tecnica_Consultar_Romaneio_Venda_Grid]
...
--	@Lojas_ID										INT = 1,
--	@Enum_Tipo_Documento_Venda_ID					INT = 0,
...
--	@Data_Geracao_Inicial							DATE = NULL,
--	@Data_Geracao_Final								DATE = NULL,
--	@Data_Liberacao_Inicial							DATE = '2018-09-04 00:00:00',
--	@Data_Liberacao_Final							DATE = '2018-10-04 00:00:00',
...
--	@Cliente_ID										VARCHAR(50) = '6E340EC1-8297-4BB5-A006-2FA49F54B5F8'
...
-------------------------------------------------------------------------------------
	...
	DECLARE @Enum_Filtro_Tipo_Exibicao_Romaneio_Id INT = 0,@Enum_Filtro_Tipo_Exibicao_Itens_Id INT = 1,@Enum_Objeto_Tipo_Peca_Id INT = 507,@Enum_Objeto_Tipo_Servico_Id INT = 508,@Enum_Objeto_Tipo_Encomenda_Id INT = 509,@Enum_Tipo_Romaneio_Troca_Id
	INT = 550,@Enum_Tipo_Romaneio_Estorno_Id INT = 648, --crédito de estorno de cartão
	@Enum_Tipo_Romaneio_Resta_Id INT = 797, --crédito restante originado de outro crédito

	@Enum_Tipo_Documento_Venda_Romaneio_Id INT = 934,@Enum_Tipo_Documento_Venda_Grupo_Venda_Id INT = 935,@Enum_Tipo_Documento_Venda_Cupom_Fiscal_Id INT = 936,@Enum_Tipo_Documento_Venda_Nota_Fiscal_Id INT = 937,@Enum_Tipo_Comanda_Comanda_Interna INT
	= 2023,@Enum_Tipo_Comanda_Comanda_Externa INT = 2024,@Enum_Tipodocumento_Nota_Fiscal INT = 568,@Enum_Tipodocumento_Cupom_Fiscal INT = 569,@Enum_Status_Romaneio_Venda_Pendente_Id INT = 1397,@Enum_Status_Romaneio_Venda_Liberado_Id INT = 1398,
	@Enum_Status_Romaneio_Venda_Cancelado_Id INT = 1399,@Enum_Status_Romaneio_Venda_Reativado_Id INT = 1400,@Enum_Status_Romaneio_Venda_Usado_Outra_Loja_Id INT = 1401,@Enum_Status_Romaneio_Venda_Inutilizado_Id INT = 1952,
	@Enum_Status_Romaneio_Venda_Grupo_Cancelado_Id INT = 1406,@Enum_Tipo_Romaneio_Licitacao_Id INT = 564,@Enum_Tipo_Origem_Interface_Fiscal_Grupo_Romaneio_Id INT = 1459,@Enum_Tipo_Transferencia_Transf_Ventec_Id INT = 1155,
	@Enum_Tempo_Em_Dias_Para_Pesquisa_Sete_Id INT = 2133,@Enum_Tempo_Em_Dias_Para_Pesquisa_Quinze_Id INT = 2134,@Enum_Tempo_Em_Dias_Para_Pesquisa_Trinta_Id INT = 2135,@Enum_Tempo_Em_Dias_Para_Pesquisa_Quarenta_Cinco INT = 2136,
	@Enum_Tempo_Em_Dias_Para_Pesquisa_Sessenta_Id INT = 2137,@Enum_Tempo_Em_Dias_Para_Pesquisa_Personalizado_Id INT = 2138
...
/*Pesquisa pelo número do documento (romaneio/ comandas/ grupo/ cupom/ NF)*/
	...
	BEGIN
		...
		IF
	(@Enum_Tipo_Comanda_Id IS NOT NULL)
		BEGIN
			...
			BEGIN
				INSERT INTO [#Romaneios_Venda_Ct_Com_Comandas]
					   ...
					   INNER JOIN [Comanda_Externa_Romaneio]
						   ON [Comanda_Externa_Romaneio].[Romaneio_Ct_Id] = [Romaneio_Venda_Ct].[Romaneio_Pre_Venda_Ct_Id]
				...
				BEGIN
					INSERT INTO [#Romaneios_Venda_Ct_Com_Comandas]
						   ...
						   INNER JOIN [Comanda_Interna_Romaneio]
							   ON [Comanda_Interna_Romaneio].[Romaneio_Ct_Id] = [Romaneio_Venda_Ct].[Romaneio_Pre_Venda_Ct_Id]
	...
	AND [Comanda_Interna_Romaneio].[Comanda_Interna_Id] = @Numero_Comanda
			...
			INSERT INTO [#Romaneio_Venda_Ct]
				   SELECT 
					   ...
					   [Romaneio_Venda_Ct_Data_Cancelamento]
				   FROM 
					   [#Romaneios_Venda_Ct_Com_Comandas] [Romaneios_Venda_Ct_Com_Comandas]
				   INNER JOIN [Romaneio_Venda_Ct]
					   ON [Romaneio_Venda_Ct].[Romaneio_Venda_Ct_Id] = [Romaneios_Venda_Ct_Com_Comandas].[Romaneio_Venda_Ct_Id]
						  AND [Romaneio_Venda_Ct].[Lojas_Id] = [Romaneios_Venda_Ct_Com_Comandas].[Lojas_Id]
				   LEFT JOIN [Romaneio_Venda_It]
					   ON [Romaneio_Venda_It].[Romaneio_Venda_Ct_Id] = [Romaneio_Venda_Ct].[Romaneio_Venda_Ct_Id]
						  AND [Romaneio_Venda_It].[Lojas_Id] = [Romaneio_Venda_Ct].[Lojas_Id]
				   LEFT JOIN [Encomenda_Venda_It]
					   ON [Encomenda_Venda_It].[Encomenda_Venda_It_Id] = [Romaneio_Venda_It].[Objeto_Id]
	...
	([Romaneio_Venda_Ct].[Lojas_Id] = @Lojas_Id
	 OR @Lojas_Id IS NULL)
	AND
	([Romaneio_Venda_Ct].[Enum_Tipo_Id] IN
	(
		SELECT 
			[Valor]
		FROM 
	...
	AND
	([Romaneio_Venda_Ct].[Enum_Status_Id] IN
	(
		SELECT 
			[Valor]
		FROM 
	...
	OR (@Cliente_Id IS NOT NULL AND @Exibir_Creditos = 1 AND [Romaneio_Venda_Ct].Enum_Status_ID = 1399)
	)
						AND
	(
	...
	([Romaneio_Venda_Ct].[Usuario_Vendedor_Id] = @Usuario_Vendedor_Id
	 OR @Usuario_Vendedor_Id IS NULL)
			AND
	(
	[Romaneio_Venda_Ct].[Enum_Status_ID] = 1399  AND [Romaneio_Venda_Ct].[Usuario_Aprovacao_Cancelamento_ID] = @Usuario_Gerente_Id
	OR
	[Romaneio_Venda_Ct].[Enum_Status_ID] <> 1399  AND [Romaneio_Venda_Ct].[Usuario_Gerente_Id] = @Usuario_Gerente_Id
	OR 
	@Usuario_Gerente_Id IS NULL 
	)
			AND
	(
	...
	AND
		(
			...
			OR 
			(@Exibir_Creditos = 1 AND (@Cliente_Id IS NOT NULL OR Enum_Status_ID = 1398))
		)
		END
			ELSE
		BEGIN

			IF(@Fabricante_Id IS NULL AND @Produto_Id IS NULL AND @Peca_Id IS NULL AND @Servico_Id IS NULL)
			BEGIN 
...
-------------------------Verificar variavel de @Cliente_Tipo_Grupo_CT_ID se esta preenchida para aplicar a validação-------------------------------------
	IF
	(@Cliente_Tipo_Grupo_Ct_Id IS NOT NULL) 
	BEGIN
		DELETE [#Romaneio_Venda_Ct]
		WHERE [#Romaneio_Venda_Ct].Cliente_Id NOT IN (	SELECT [Tmp].CLIENTE_ID
														FROM 
															[#Romaneio_Venda_Ct] [Tmp]
														...
														WHERE 
														ISNULL([Cliente_Tipo_Grupo_It].[Cliente_Tipo_Grupo_CT_ID],0) = @Cliente_Tipo_Grupo_Ct_Id)
	END
-------------------------Verificar variavel de @Cliente_Tipo_Grupo_CT_ID se esta preenchida para aplicar a validação-------------------------------------
...
-------------------------Atualizar o campo Transferencias_CT_ID para Romaneios que tenham vinculo com Transferencia--------------------
	...
	INNER JOIN [Transferencia_Ct]
		 ON [Transferencia_Ct].[Objeto_Origem_Id] = [Romaneio_Venda_Ct].[Romaneio_Pre_Venda_Ct_Id]
...
/*#ROMANEIO*/
		...
		SELECT DISTINCT 
			...
			CASE
				WHEN [Romaneio_Venda_Ct].[Enum_Tipo_Id] NOT IN(@Enum_Tipo_Romaneio_Troca_Id,@Enum_Tipo_Romaneio_Estorno_Id,@Enum_Tipo_Romaneio_Resta_Id) THEN [Dbo].[Fun_Calcula_Percentual_Desconto]([Romaneio_Venda_Ct].[Romaneio_Venda_Ct_Valor_Pago],
				[Romaneio_Venda_Ct].[Romaneio_Venda_Ct_Valor_Lista]
																																																	 )
				ELSE 0
		END AS [% Desc],
			...
			CASE
				...
				ELSE [Enumerado_Cancelamento].[Enum_Extenso]+' - '
																	  END+COALESCE([Romaneio_Venda_Ct].[Romaneio_Venda_Ct_Motivo_Venda_Especial],[Romaneio_Venda_Ct].[Romaneio_Venda_Ct_Motivo_Troca],[Romaneio_Venda_Ct].
																	  [Romaneio_Venda_Ct_Motivo_Aprovacao_Credito],[Romaneio_Venda_Ct].[Romaneio_Venda_Ct_Motivo_Cancelamento]) AS [Motivo],
		...
		FROM 
			[#Romaneio_Venda_Ct] [Romaneio_Venda_Ct]
		LEFT JOIN [Romaneio_Venda_It]
			ON [Romaneio_Venda_It].[Romaneio_Venda_Ct_Id] = [Romaneio_Venda_Ct].[Romaneio_Venda_Ct_Id]
			   AND [Romaneio_Venda_It].[Lojas_Id] = [Romaneio_Venda_Ct].[Lojas_Id]
		INNER JOIN [Enumerado] [Enumerado_Tipo]
			ON [Enumerado_Tipo].[Enum_Id] = [Romaneio_Venda_Ct].[Enum_Tipo_Id]
		INNER JOIN [Enumerado] [Enumerado_Status]
		...
		LEFT JOIN [Condicao_Pagamento]
			ON [Romaneio_Venda_Ct].[Condicao_Pagamento_Id] = [Condicao_Pagamento].[Condicao_Pagamento_Id]
		...
		LEFT JOIN [Pacote_Venda_Ct]
			ON [Pacote_Venda_Ct].[Romaneio_Ct_Id] = [Romaneio_Venda_Ct].[Romaneio_Pre_Venda_Ct_Id]
			   AND [Pacote_Venda_Ct].[Lojas_Id] = [Romaneio_Venda_Ct].[Lojas_Id]
		LEFT JOIN [Encomenda_Venda_Ct]
			ON [Encomenda_Venda_Ct].[Romaneio_Ct_Id] = [Romaneio_Venda_Ct].[Romaneio_Pre_Venda_Ct_Id]
			   AND [Encomenda_Venda_Ct].[Lojas_Id] = [Romaneio_Venda_Ct].[Lojas_Id]
		LEFT JOIN [Romaneio_Cancelado_Ct]
			ON [Romaneio_Cancelado_Ct].[Romaneio_Pre_Venda_Ct_Id] = [Romaneio_Venda_Ct].[Romaneio_Pre_Venda_Ct_Id]
			   AND [Romaneio_Cancelado_Ct].[Lojas_Id] = [Romaneio_Venda_Ct].[Lojas_Id]
		LEFT JOIN [Enumerado] [Enumerado_Cancelamento]
...
/*#ITENS*/
		...
		SELECT DISTINCT 
			...
			CASE
				WHEN [Romaneio_Venda_Ct].[Enum_Tipo_Id] NOT IN(@Enum_Tipo_Romaneio_Troca_Id,@Enum_Tipo_Romaneio_Estorno_Id,@Enum_Tipo_Romaneio_Resta_Id) THEN [Dbo].[Fun_Calcula_Percentual_Desconto]([Romaneio_Venda_It].[Romaneio_Venda_It_Preco_Pago],
		...
		LEFT JOIN [Encomenda_Venda_It]
			ON [Encomenda_Venda_It].[Encomenda_Venda_It_Id] = [Romaneio_Venda_It].[Objeto_Id]
			   AND [Encomenda_Venda_It].[Lojas_Id] = [Romaneio_Venda_It].[Lojas_Id]
		LEFT JOIN [Vw_Peca]
			ON [Vw_Peca].[Peca_Id] = [Romaneio_Venda_It].[Objeto_Id]
			   AND [Romaneio_Venda_It].[Enum_Objeto_Tipo_Id] = @Enum_Objeto_Tipo_Peca_Id
		LEFT JOIN [Servico]
			ON [Servico].[Servico_Id] = [Romaneio_Venda_It].[Objeto_Id]
