...
-- </history>      
-------------------------------------------------------------------------------      
CREATE PROCEDURE [dbo].[P_Compras_Calcular_Analise_Duracao_Estoque]  
 (  
...
--SET @ID_Analise_Cotacao           = 440661    
-------------------------------------------------------------------------------      
  
 SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED  
 SET NOCOUNT ON  
  
 DECLARE @Enum_Tipo_Movimentacao_Venda INT = 93,  
 ...
 DECLARE @Lojas_Filtro TABLE  
  (  
   Lojas_Id INT  
  )  
 DECLARE @Pecas_Fabricante_Alternativo TABLE  
  (  
   Peca_Id INT,  
   Fabricante_Alternativo_Ct_Id INT  
  )  
 DECLARE @Curvas_Filtro TABLE  
  (  
   Curva CHAR(1)  
  )  
 CREATE TABLE #Lojas  
  (  
   Lojas_Id INT,  
   Lojas_Tipo VARCHAR(50)  
  )      
      
--==Lojas selecionadas na duraçao de estoque==--      
 INSERT INTO @Lojas_Filtro(  
  Lojas_Id)  
  
 SELECT   
  Lojas_Id  
 FROM   
  Analise_Cotacao_Loja_Parametros  
...
--==Pega curvas selecionadas na tela==--      
 ...
 BEGIN  
  INSERT INTO @Curvas_Filtro(  
   Curva)  
  VALUES  
   (  
    SUBSTRING(@Curvas,@Aux_Curvas,1)  
   )  
  SET @Aux_Curvas =  
        @Aux_Curvas + 2  
 END      
...
--==Apaga dados para recalcular==--      
 ...
 WHERE   
  Analise_Cotacao_Id = @Id_Analise_Cotacao  
...
--==Todas as peças dos grupos de alternativos o qual a peça o fabricante imformado seja o fabricante da peça principal do grupo==--      
 ...
 WHERE Fabricante_Alternativo_It_Ativo = 1  
  ...
  (  
   ...
   WHERE  
    (Peca.Fabricante_Id = @Id_Fabricante  
     OR @Id_Fabricante = 0)  
    AND  
    (Peca.Produto_Id = @Id_Produto  
     OR @Id_Produto = 0)  
    AND (Peca.Peca_Isativo = 1)  
    AND  
    (ISNULL(Peca_Curva_Mista,'E') IN(SELECT Curva FROM   
       @Curvas_Filtro  
    ))  
 )      
...
------------------------------------------------------------------------------------------------------------------------      
 ...
 SELECT DISTINCT   
  ...
  Dbo.Fun_Retorna_Zero_Para_Valores_Negativos(ISNULL(E.Estoque_Qtde,0)) - ISNULL(  
                        (  
                         ...
                         FROM   
                          Estoque_Divergencia AS Ed  
                         WHERE Usuario_Tratamento_Id IS NULL  
                         AND Ed.Objeto_Id = Vw.Peca_Id  
                         AND Ed.Lojas_Id = #Lojas.Lojas_Id  
                       ),0) AS Analise_Cotacao_Loja_Qtde_Estoque,  
  (  
   (  
    SELECT   
     ISNULL((SUM(  
        ISNULL(Estoque_Transito_Qtde_Transito,0) + ISNULL(Estoque_Transito_Qtde_Processamento,0))),0)  
    FROM   
     Estoque_Transito  
    WHERE Estoque_Transito.Peca_Id = Vw.Peca_Id  
       AND Estoque_Transito.Loja_Destino_Id = #Lojas.Lojas_Id  
       AND  
     (Estoque_Transito_Qtde_Transito > 0  
      OR Estoque_Transito_Qtde_Processamento > 0)  
       AND  
     (Estoque_Transito.Loja_Origem_Id NOT IN(@Loja_Garantia_Id))  
    AND  
     (Estoque_Transito.Loja_Destino_Id NOT IN(@Loja_Garantia_Id))  
    AND  
 ...
 --ISNULL(Estoque_Calculo_Maximo_Qtde, 0)         AS Analise_Cotacao_Loja_Qtde_Estoque_Maximo,      
  CASE  
   WHEN  
  ((ISNULL(Peca_Estoque_Totalmente_Lojas,0) = 1)  
  ...
  END AS Analise_Cotacao_Loja_Qtde_Estoque_Maximo,      
   ...
   --          THEN ISNULL((SELECT    Estoque_Calculo.Estoque_Calculo_Maximo_Qtde      
   ...
   --          ELSE ISNULL((SELECT    Estoque_Calculo.Estoque_Calculo_Maximo_Qtde      
   --                       FROM      Estoque_Calculo      
   --                       WHERE     Estoque_Calculo.Peca_ID = vw.Peca_ID      
   --                                 AND Estoque_Calculo.Loja_ID = #Lojas.Lojas_ID      
   --                                 AND Estoque_Calculo.Loja_ID IN (      
 ...
 AND (Estoque_Calculo.Loja_Id = #Lojas.Lojas_Id)  
 AND  
  (Estoque_Calculo.Loja_Id IN (SELECT Lojas_Id FROM @Lojas_Filtro)  
   OR Lojas_Tipo = ('CD'))  
 LEFT JOIN Fabricante_Alternativo_It  
  ON Fabricante_Alternativo_It.Peca_Id = Vw.Peca_Id  
     ...
     AND Fabricante_Alternativo_It.Fabricante_Alternativo_It_Ativo = 1      
 --AND      
 --Fabricante_Alternativo_IT_Peca_Principal = 1      
 ...
 WHERE  
  (Vw.Fabricante_Id = @Id_Fabricante  
   OR @Id_Fabricante = 0)  
  AND  
  (Vw.Produto_Id = @Id_Produto  
   OR @Id_Produto = 0)  
  AND  
  (Fabricante_Alternativo_It.Fabricante_Alternativo_Ct_Id IS NULL)  
  AND  
  (ISNULL(Vw.Peca_Curva_Mista,'E') IN(SELECT Curva FROM @Curvas_Filtro))  
...
------------------------------------------------------------------------------------------------------------------------------------------------      
 ...
 SELECT DISTINCT   
  ...
  (  
   (  
    SELECT   
     ISNULL((SUM(  
        ISNULL(Estoque_Transito.Estoque_Transito_Qtde_Transito,0) + ISNULL(Estoque_Transito.Estoque_Transito_Qtde_Processamento,0))),0)  
    FROM   
     Estoque_Transito  
    WHERE Estoque_Transito.Peca_Id = Vw.Peca_Id  
       AND Estoque_Transito.Loja_Destino_Id = #Lojas.Lojas_Id  
  ...
  ( SELECT TOP 1  
   ...
   FROM  
    Fabricante_Alternativo_IT  
   JOIN Peca ON  
     Peca.Peca_ID = Fabricante_Alternativo_IT.Peca_ID  
   WHERE  
    Fabricante_Alternativo_IT.Fabricante_Alternativo_CT_ID = Pecas_Fabricante_Alternativo.Fabricante_Alternativo_Ct_Id  
   AND  
    Fabricante_Alternativo_IT.Fabricante_Alternativo_IT_Ativo = 1  
   AND  
    Fabricante_Alternativo_IT.Fabricante_Alternativo_IT_Peca_Principal = 1 )         AS Peca_Comprar  
 INTO   
  #Analise_Cotacao_Loja_Fabricante_Alternativo  
 FROM   
  Peca AS Vw  
 FULL OUTER JOIN #Lojas  
  ON #Lojas.Lojas_Id = #Lojas.Lojas_Id  
 ...
 LEFT OUTER JOIN Estoque_Calculo  
  ...
  AND (Estoque_Calculo.Loja_Id = #Lojas.Lojas_Id)  
  AND  
   (Estoque_Calculo.Loja_Id IN(SELECT Lojas_Id FROM @Lojas_Filtro) OR Lojas_Tipo = ('CD'))  
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------      
 ...
 SELECT   
  ...
  Peca_Id,  
  Lojas_Id,  
  CASE WHEN Lojas_ID IN (SELECT Lojas_ID FROM #Lojas WHERE Lojas_Tipo = 'CD') THEN   
   0   
  ELSE  
   Analise_Cotacao_Loja_Qtde_Venda_Media   
...
------------------------------------------------------------------------------------------------------------------------------------    
 ...
 INTO   
  #Dados_Calculados_Peca_Fabricante_Alternativo  
 FROM   
  Analise_Cotacao_Loja  
 WHERE Analise_Cotacao_Id = @Id_Analise_Cotacao  
 ...
 SET   
  ...
  --        SELECT   
  --         SUM(  
  --          ISNULL(Analise_Cotacao_Loja.Analise_Cotacao_Loja_Qtde_Estoque,0) + ISNULL(Analise_Cotacao_Loja.Analise_Cotacao_Loja_Qtde_Estoque_Transito_Entrada,0))  
  --        FROM   
  --         Analise_Cotacao_Loja  
...
------------------------------------------------------------------------------------------------------------------------------------      
 ...
 SET Analise_Cotacao_Loja_Qtde_Calculada =   
  CASE WHEN Analise_Cotacao_Loja.Fabricante_Alternativo_Ct_Id IS NULL THEN Dbo.Fun_Retorna_Zero_Para_Valores_Negativos(  
    Analise_Cotacao_Loja.Analise_Cotacao_Loja_Qtde_Estoque_Maximo - (Analise_Cotacao_Loja.Analise_Cotacao_Loja_Qtde_Estoque + Analise_Cotacao_Loja.Analise_Cotacao_Loja_Qtde_Estoque_Transito_Entrada))  
  ELSE   
   CASE WHEN  
  (Temp.Peca_Principal_Id = Analise_Cotacao_Loja.Peca_Id  
   AND  
   (Temp.Qtde_Venda_Media > 0  
    OR Temp.Qtde_Venda_Periodo > 0)) THEN Dbo.Fun_Retorna_Zero_Para_Valores_Negativos(  
 ...
 WHERE   
  Analise_Cotacao_Loja.Analise_Cotacao_Id = @Id_Analise_Cotacao  
 AND  
  Analise_Cotacao_Loja.Lojas_ID NOT IN (SELECT Lojas_ID FROM #Lojas WHERE Lojas_Tipo = 'CD')  
 ...
 SELECT DISTINCT   
  ...
  (  
   SELECT TOP 1   
    Peca_Embalagem_Id  
   FROM   
    Peca_Embalagem AS Pe  
   WHERE Pe.Peca_Id = Analise_Cotacao_Loja.Peca_Id  
      AND Pe.Peca_Embalagem_Ativo = 1  
      AND Pe.Peca_Embalagem_Compra = 1  
    ORDER BY   
    Pe.Peca_Embalagem_Quantidade  
  ) AS Peca_Embalagem_Compra_Id,  
  CASE  
   WHEN(Peca.Peca_Comprar = 0) THEN 0  
   ELSE Dbo.Fun_Retorna_Zero_Para_Valores_Negativos  
    ( --Peças sem alternativos      
     Sum(Analise_Cotacao_Loja_Qtde_Calculada) - ISNULL((  
                    ...
                    FROM   
                     #Analise_Cotacao_Loja  
                    WHERE #Analise_Cotacao_Loja.Lojas_Id IN (SELECT Lojas_ID FROM #Lojas WHERE Lojas_Tipo = 'CD') AND @Loja_Nova = 0  
                   AND #Analise_Cotacao_Loja.Peca_Id = Analise_Cotacao_Loja.Peca_Id  
                   AND #Analise_Cotacao_Loja.Analise_Cotacao_Id = @Id_Analise_Cotacao  
                  ),0))   
  ...
  CASE  
   WHEN ISNULL(Fabricante_Alternativo_Ct_Id,0) = 0 THEN FLOOR((CASE  
                   WHEN SUM(ISNULL(Analise_Cotacao_Loja_Qtde_Venda_Media,0)) = 0  
                     OR  
                     (  
                      SELECT   
                       SUM(Dbo.Fun_Retorna_Zero_Para_Valores_Negativos(#Analise_Cotacao_Loja.Analise_Cotacao_Loja_Qtde_Estoque)) + SUM(Dbo.Fun_Retorna_Zero_Para_Valores_Negativos(  
                       #Analise_Cotacao_Loja.Analise_Cotacao_Loja_Qtde_Estoque_Transito_Entrada  
                                                   ))  
                      FROM   
                       #Analise_Cotacao_Loja  
                      WHERE #Analise_Cotacao_Loja.Peca_Id = Analise_Cotacao_Loja.Peca_Id  
                    ) <= 0 THEN 0  
                   ELSE  
                     (  
                      SELECT   
                     ...
                     SUM(ISNULL(Analise_Cotacao_Loja_Qtde_Venda_Media,0)) / 30)  
                  END))  
   ELSE FLOOR((CASE  
       WHEN SUM(ISNULL(Analise_Cotacao_Loja_Qtde_Venda_Media,0)) = 0  
         OR  
         (  
          SELECT   
           SUM(Dbo.Fun_Retorna_Zero_Para_Valores_Negativos(#Analise_Cotacao_Loja_Fabricante_Alternativo.Analise_Cotacao_Loja_Qtde_Estoque)) + SUM(Dbo.Fun_Retorna_Zero_Para_Valores_Negativos(  
           #Analise_Cotacao_Loja_Fabricante_Alternativo.Analise_Cotacao_Loja_Qtde_Estoque_Transito_Entrada  
                                            ))  
          FROM   
           #Analise_Cotacao_Loja_Fabricante_Alternativo  
          WHERE #Analise_Cotacao_Loja_Fabricante_Alternativo.Peca_Id = Analise_Cotacao_Loja.Peca_Id  
        ) <= 0 THEN 0  
       ELSE  
         (  
          SELECT   
  ...
  ISNULL(  
      (  
       SELECT   
        SUM(Solicitacao_Garantia_It_Quantidade)  
       FROM   
        Solicitacao_Garantia_Ct  
       ...
       AND Solicitacao_Garantia_It.Objeto_Origem_Id = Analise_Cotacao_Loja.Peca_Id  
  ...
  CASE  
   WHEN  
    (ISNULL(  
      ...
      (CASE  
        WHEN(  
         SUM(Analise_Cotacao_Loja_Qtde_Venda_Media) / 30) <= 0  
         OR SUM(  
        ...
        ELSE ISNULL((  
           SUM(  
            Analise_Cotacao_Loja_Qtde_Estoque + Analise_Cotacao_Loja_Qtde_Estoque_Transito_Entrada) / (  
                                    SUM(Analise_Cotacao_Loja_Qtde_Venda_Media) / 30)),0)  
       END) - @Analise_Cotacao_Dias_Excedentes) * ISNULL((  
                     SUM(Analise_Cotacao_Loja_Qtde_Venda_Media) / 30),0),0) > 0  
    ) THEN ISNULL(  
         (  
         (CASE  
           WHEN(  
           ...
           ELSE ISNULL((  
              SUM(  
               Analise_Cotacao_Loja_Qtde_Estoque + Analise_Cotacao_Loja_Qtde_Estoque_Transito_Entrada) / (  
                                    SUM(Analise_Cotacao_Loja_Qtde_Venda_Media) / 30)),0)  
          END) - @Analise_Cotacao_Dias_Excedentes) * ISNULL((  
                     SUM(Analise_Cotacao_Loja_Qtde_Venda_Media) / 30),0),0)  
     ELSE 0  
           ...
           ELSE ISNULL((  
              SUM(  
               Analise_Cotacao_Loja_Qtde_Estoque + Analise_Cotacao_Loja_Qtde_Estoque_Transito_Entrada) / (  
                                    SUM(Analise_Cotacao_Loja_Qtde_Venda_Media) / 30)),0)  
          END) - @Analise_Cotacao_Dias_Excedentes) * ISNULL((  
                     SUM(Analise_Cotacao_Loja_Qtde_Venda_Media) / 30),0) * ISNULL(Peca_Preco_Custo_Reposicao,0),0)  
     ELSE 0  
  ...
  COALESCE(Peca_Preco.Peca_Preco_Valor_Fabrica,Peca_Preco_Custo_Reposicao,0)   AS Analise_Cotacao_Peca_Preco_Fabrica  
 INTO   
  #Analise_Cotacao_Peca  
 FROM   
  Analise_Cotacao_Loja  
 INNER JOIN Peca  
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------      
 ...
 SELECT   
  ...
  Qtde_Calculada = CASE WHEN Peca_Comprar = 0 THEN 0  
        ELSE Dbo.Fun_Retorna_Zero_Para_Valores_Negativos((SUM(  
         Analise_Cotacao_Loja_Qtde_Estoque_Maximo - (Analise_Cotacao_Loja_Qtde_Estoque + Analise_Cotacao_Loja_Qtde_Estoque_Transito_Entrada))))  
        END,  
 ...
 INTO   
  #Analise_Cotacao_Loja_Fabricante_Alternativo_Agrupado  
 FROM   
  #Analise_Cotacao_Loja_Fabricante_Alternativo  
 WHERE Fabricante_Alternativo_Ct_Id IS NOT NULL  
    AND #Analise_Cotacao_Loja_Fabricante_Alternativo.Lojas_Id NOT IN (SELECT Lojas_ID FROM #Lojas WHERE Lojas_Tipo = 'CD')  
 GROUP BY   
  Fabricante_Alternativo_Ct_Id,  
  Lojas_Id,  
  Peca_Comprar    
...
--select * from #Analise_Cotacao_Loja_Fabricante_Alternativo_Agrupado where Fabricante_Alternativo_CT_ID = 4151      
 ...
 SELECT   
  ...
  (  
   SUM(Qtde_Calculada) - ISNULL((  
            ...
            FROM   
             #Analise_Cotacao_Loja_Fabricante_Alternativo AS Subquery  
            WHERE Subquery.Lojas_Id IN (SELECT Lojas_ID FROM #Lojas WHERE Lojas_Tipo = 'CD') AND @Loja_Nova = 0  
 ...
 INTO   
  #Analise_Cotacao_Peca_Fabricante_Alternativo_Agrupado  
 FROM   
  #Analise_Cotacao_Loja_Fabricante_Alternativo_Agrupado  
 WHERE Fabricante_Alternativo_Ct_Id IS NOT NULL  
 GROUP BY   
 ...
 INNER JOIN Peca  
  ...
  (  
   SELECT   
    It.Peca_Id  
   FROM   
    Fabricante_Alternativo_It AS It  
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------      
 ...
 UPDATE Analise_Cotacao_Peca  
 SET Analise_Cotacao_Peca_Qtde_Calculada = 0,Analise_Cotacao_Peca_Qtde_Confirmada_Analise = 0  
 WHERE   
  Analise_Cotacao_Id = @Id_Analise_Cotacao  
  AND Fabricante_Alternativo_Ct_Id IS NOT NULL  
...
----------Quando a analise de duração tiver origem da pendencia de compra os itens devem ser identificados----------      
 ...
 BEGIN  
  ...
  WHERE   
   Temp_Analise_Cotacao_Peca.Analise_Cotacao_Id = @Id_Analise_Cotacao  
