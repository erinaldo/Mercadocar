...
--   Modificado
--  [fmoraes]   -16/05/2019 Modified
--   Vinculado os registros com as tabelas Conferencia_Separacao, Conferencia_CT
--   alterando o LEFT JOIN Pre_Recebimento_Volume_CT Volume_Conferencia para
--...referenciar a coluna Conferencia_CT.Objeto_Origem_ID retornando o campo
--...Fornecedor_Nome correto.
-- </history>
-------------------------------------------------------------------------------
alter PROCEDURE [dbo].[p_CD_Consultar_Divergencia_Entrada_Grid_Chamado_177962] (
  @Lojas_Origem_ID           INT,
...
--SET @Enum_Tipo_ID            = NULL -- 2530
--SET @Date_Inicial            = '2019-04-24'--'1900-01-01'
--SET @Date_Final              = '2019-05-06'--'1900-01-01'
--SET @Fabricante_ID           = 1945 --NULL
--SET @Produto_ID              = 169  --NULL
--SET @Peca_ID                 = NULL --62659 --44274
--SET @Tipo_Pre_Recebimento_ID = NULL --168
...
-----------SE A LOJA FOR CENTRO DE DITRIBUIÇÃO CONSULTA DIVERGENCIAS DE CONFERENCIA OU SEPARAÇÃO-------------
  ...
  BEGIN
    SELECT DISTINCT
      ...
      CASE
        ...
        THEN (SELECT TOP 1 'Grupo.Vol.: ' +
          ...
          WHERE Conferencia_Separacao_ID = Pre_Recebimento_Divergencia.Objeto_Origem_ID)
        WHEN Pre_Recebimento_Divergencia.Enum_Tipo_ID = @Enum_Tipo_Divergencia_Entrada_Conferencia_Cross_ID
        THEN (SELECT TOP 1 'Grupo.Vol.: ' +
      ...
      CASE
        WHEN Pre_Recebimento_Divergencia.Enum_Tipo_ID = @Tipo_Divergencia_Entrada_Conferencia
          ...
          AND ISNULL(Pre_Recebimento_Divergencia_Solucao.Objeto_Destino_ID, 0) <> 0
        THEN 'Grupo: ' +
            CONVERT(VARCHAR, CONVERT(VARCHAR, ISNULL(volume_destino.Pre_Recebimento_Grupo_ID, '')) + '.') +
            CONVERT(VARCHAR, ISNULL(volume_destino.Pre_Recebimento_Volume_CT_Numero, ''))
        ELSE ''
      END AS Destino,

      CASE
        WHEN Pre_Recebimento_Divergencia.Enum_Tipo_ID
        IN (@Enum_Tipo_Divergencia_Entrada_Separacao_Cross_ID, @Enum_Tipo_Divergencia_Entrada_Conferencia_Cross_ID)
        THEN Enum_Tipo_Pre_Conferencia.Enum_Extenso
        ELSE EnumTipoPre.Enum_Extenso
      END AS Enum_Extenso_Tipo_Pre_Recebimento,

	  COALESCE(Fornecedor_Conferencia.Fornecedor_Nome, Fornecedor_Separacao.Fornecedor_Nome, Fornecedor.Fornecedor_Nome) AS Fornecedor_Nome,
      ...
      CASE
        WHEN vw_Peca.Fabricante_ID IS NOT NULL
        THEN dbo.fun_Concatena_Comprador_Por_Fabricante(vw_Peca.Fabricante_ID)
        ELSE ''
      END AS Compradores,
      (SELECT MIN(Pre_Recebimento_Divergencia_Parecer.Pre_Recebimento_Divergencia_Parecer_Data_Proxima_Cobranca)
    ...
    LEFT JOIN Pre_Recebimento_Volume_CT Volume_Conferencia
	  ...
	  AND Pre_Recebimento_Divergencia.Enum_Tipo_ID = @Enum_Tipo_Divergencia_Entrada_Conferencia_Cross_ID
    ...
    LEFT JOIN Conferencia_Separacao
      ON Pre_Recebimento_Divergencia.Objeto_Origem_ID = Conferencia_Separacao.Conferencia_Separacao_ID
	  AND Pre_Recebimento_Divergencia.Enum_Tipo_ID = @Enum_Tipo_Divergencia_Entrada_Separacao_Cross_ID
    LEFT JOIN Conferencia_CT
      ON Conferencia_Separacao.Conferencia_CT_ID = Conferencia_CT.Conferencia_CT_ID
    LEFT JOIN Pre_Recebimento_Volume_CT Volume_Separacao 
	  ON Conferencia_CT.Objeto_Origem_ID = Volume_Separacao.Pre_Recebimento_Volume_CT_ID

    LEFT JOIN Pre_Recebimento_CT Pre_Recebimento_Separacao
      ON Volume_Separacao.Pre_Recebimento_Grupo_ID = Pre_Recebimento_Separacao.Pre_Recebimento_Grupo_ID
    LEFT JOIN Fornecedor Fornecedor_Separacao
      ON Pre_Recebimento_Separacao.Forn_ID = Fornecedor_Separacao.Forn_ID
    ...
    AND (Pre_Recebimento_Divergencia.Enum_Status_ID
      IN (SELECT DISTINCT Enum_ID FROM @Status))
    AND (Pre_Recebimento_Divergencia.Enum_Tipo_ID = ISNULL(@Enum_Tipo_ID, Pre_Recebimento_Divergencia.Enum_Tipo_ID))
    AND (Pre_Recebimento_Divergencia.Pre_Recebimento_Divergencia_Data_Geracao
      BETWEEN @Date_Inicial AND @Date_Final OR @Date_Inicial IS NULL)
    ...
    --AND (Pre_Recebimento_CT.Enum_Tipo_ID = @Tipo_Pre_Recebimento_ID OR @Tipo_Pre_Recebimento_ID IS NULL)
    AND (@Tipo_Pre_Recebimento_ID IS NULL
    OR (Pre_Recebimento_Divergencia.Enum_Tipo_ID
      IN (@Enum_Tipo_Divergencia_Entrada_Separacao_Cross_ID, @Enum_Tipo_Divergencia_Entrada_Conferencia_Cross_ID)
    AND Pre_Recebimento_Conferencia.Enum_Tipo_ID = @Tipo_Pre_Recebimento_ID)
    OR (Pre_Recebimento_Divergencia.Enum_Tipo_ID
      NOT IN (@Enum_Tipo_Divergencia_Entrada_Separacao_Cross_ID, @Enum_Tipo_Divergencia_Entrada_Conferencia_Cross_ID)
    ...
    FROM Pre_Recebimento_Divergencia_Parecer
    WHERE Pre_Recebimento_Divergencia_Parecer.Pre_Recebimento_Divergencia_ID = Pre_Recebimento_Divergencia.Pre_Recebimento_Divergencia_ID
    AND Pre_Recebimento_Divergencia_Parecer.Enum_Status_ID
      IN (SELECT Enum_ID FROM @Status_Parecer)
    AND (Pre_Recebimento_Divergencia_Parecer.Pre_Recebimento_Divergencia_Parecer_Data_Proxima_Cobranca
      BETWEEN @Date_Inicial_Parecer AND @Date_Final_Parecer)
    OR @Date_Inicial_Parecer IS NULL)
    OR (SELECT COUNT(*) FROM @Status_Parecer) = 0
    )
  ...
  BEGIN
    SELECT DISTINCT
      ...
      CASE
        ...
        WHEN Pre_Recebimento_Divergencia.Enum_Tipo_ID = @Tipo_Divergencia_Entrada_Conferencia
          AND Pre_Recebimento_Divergencia_Qtde < 0
        THEN 'Grupo: ' + CONVERT(VARCHAR, CONVERT(VARCHAR, Pre_Recebimento_Volume_CT.Pre_Recebimento_Grupo_ID) + '.' + CONVERT(VARCHAR, Pre_Recebimento_Volume_CT.Pre_Recebimento_Volume_CT_Numero))
        WHEN (Pre_Recebimento_Divergencia.Enum_Tipo_ID = @Tipo_Divergencia_Entrada_Conferencia
          AND ISNULL(Pre_Recebimento_Divergencia.Objeto_Origem_ID, 0) <> 0)
          AND Pre_Recebimento_Divergencia_Qtde > 0
        THEN 'Romaneio: ' + CONVERT(VARCHAR, Pre_Recebimento_Divergencia.Objeto_Origem_ID)
        WHEN Pre_Recebimento_Divergencia.Enum_Tipo_ID = @Tipo_Divergencia_Entrada_Guarda
        THEN 'Grupo: ' +
          CONVERT(VARCHAR, CONVERT(VARCHAR, Pre_Recebimento_Volume_CT.Pre_Recebimento_Grupo_ID) + '.' +
          CONVERT(VARCHAR, Pre_Recebimento_Volume_CT.Pre_Recebimento_Volume_CT_Numero))
        ELSE ''
      END AS Origem,
      ISNULL(Pre_Recebimento_Divergencia.Lojas_Destino_ID, 0) AS Lojas_Destino_ID,
      LojaDestino.Lojas_NM AS Loja_Destino,
